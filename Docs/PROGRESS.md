...
- 交付产物重建：源变更后 `make release VERSION=0.2.0-p1` 重新打包；消费者侧校验通过（unzip → `shasum -a 256 -c SHA256SUMS.txt` OK → `codesign --verify --deep --strict` OK，adhoc universal）；最终产物 SHA `6f709e27…`（随源码变更重建）。`dist/` 为 gitignore 本地产物，不入库。
...
- 本机真实 Provider 验收反馈修复（2026-09-09，两次提交）：
  - `45a5987 fix: surface actionable Chinese errors when profile API keys are missing`——真实 Profile 触发 Action 时若 `${keychain:账户}` 条目缺失/`${env:变量}` 未设置/`apiKey` 为空，之前只显示裸 `SecretResolverError error 1`；现经 `SecretResolver.resolvedProfileKey(profileId:reference:)` 映射为带原因与解决步骤的中文 `ConfigError`（ActionRunner 与聊天提交路径同用），UI/CLI 直接展示。新增 `Tests/SecretResolverTests.swift` 3 项。
  - `a2e7f4b chore: ship clean default config and stop settings editor example appending`——出厂 `DefaultConfig.toml` 移除 Profile/Action 示例块（每项至多一行注释、无残留字段）；Settings 编辑器不再自动拼接“高级配置参考”示例段（所见即磁盘文件，避免重复示例与注释落盘）；CONFIG.md 措辞与实际语义对齐（仅 `attachTo` 报错；`launchAtLogin`/`toggleHotkey`/`tails` 残留被忽略并建议手动删除）。新增出厂模板洁净度测试 1 项。
  - 门禁：**116 项全绿**（112 + 4）；`make lint` 全绿；`make release VERSION=0.2.0-p1` 产物 SHA `6f709e27…`，消费者侧校验通过（unzip/shasum/codesign OK），包内 DefaultConfig 已为精简版。
- 本机验收反馈修复（签名与卸载，2026-09-09）：
  - `29a58ad build: sign Debug/Release with local Apple Development certificate by default`——交付签名由 ad-hoc 改为默认本机 Apple Development 证书（`WCHFR3G7VB` / Team `3CSL8ZN3AN`）：xcconfig `CODE_SIGN_IDENTITY`/`DEVELOPMENT_TEAM` 落位，Makefile 新增 `SIGN_IDENTITY` 变量（`SIGN_IDENTITY=-` 回退 ad-hoc）与 `sign-check` 预检，Debug/Release 均稳定签名。同时修复了该证书在用户域的自定义信任覆盖（`security remove-trusted-cert` 恢复系统默认），否则 codesign 报 “Invalid trust settings”。验证：Debug 与 dist 的 designated requirement 均为 `identifier … and anchor apple generic and certificate leaf[CN]=Apple Development: …(WCHFR3G7VB)`（跨重建稳定）→ 辅助功能/钥匙串授权不再因重建失效；`codesign --verify --deep --strict` 通过。
  - `6991b53 docs: add uninstall residue guide and cleanup script`——卸载无残留：新增根 `uninstall.sh`（退出 App → 删除 `~/.config/consolepilot`、`~/Library/Application Support/Consolepilot`、Caches/Logs/Preferences 域 → `tccutil reset Accessibility` → 钥匙串默认保留并给出删除命令）与 `Docs/UNINSTALL.md`（残留盘点表 + Apple File System/UserDefaults/Container 行业规范参考），`make uninstall` 入口；README/PLAN/GATE0/ACCEPTANCE 同步签名与卸载语义。
  - 门禁：**116 项全绿**；`make lint` 全绿；最新交付产物 `Consolepilot-0.2.0-p1.zip` SHA `f9e01210…`（Apple Development 签名，Team `3CSL8ZN3AN`），消费者侧校验通过。
- 本机验收反馈修复（Mock 误报缺密钥 + 设置「初始化」，2026-09-09）：
  - 修复一：聊天提交路径与 ActionRunner 的密钥判定不一致——默认配置（无 Profile/回环/`allowRealProvider=false`）走本地 Mock 时，`ConsolepilotRootView.submit` 仍无条件解析 Profile API Key，导致首次安装即报「Profile「mock」未配置 apiKey」。现与 ActionRunner 同规则：实际绑定 Provider 为 `MockAIProvider` 时不解析密钥（`apiKey=""`），仅远程执行才要求密钥；Provider 改为提交时捕获一次并与流执行同一实例，消除判定与执行间的类型漂移。场景 A 复测：默认 Mock 下可直接对话，不再报缺密钥。
...
- 本机功能新增（Action/Profile 级上下文上限 `maxContextBytes`，2026-09-09）：
  - 需求：增加 Action 级上下文限制字段，Provider（Profile）级也要加；Action 级显式设置时覆盖 Profile 级，未设置回退 Profile；按 UTF-8 **字节**裁剪；默认 128 KiB。建议并采纳：优先整条丢弃最早历史、只在最新一条单独仍超时才在字符边界截断其正文（绝不拆多字节、绝不丢当前轮次）、下限 1024（1 KiB）由校验拦截。
  - Schema：`Profile.maxContextBytes: Int`（默认 `131072`）；`Action.maxContextBytes: Int?`（可选，覆盖 Profile）；ConfigLoader/ConfigValidator 同步（解析默认、`<1024` 报 `valueOutOfRange`）；`DefaultConfig.toml` 注释示例补 `maxContextBytes`（Profile 131072 / Action 65536 示例）。
